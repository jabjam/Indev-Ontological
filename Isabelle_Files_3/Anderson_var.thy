...
consts A :: "\<mu> \<Rightarrow> \<sigma>"

axiomatization  where Affectasymm : "\<lfloor> \<^bold>\<not>(x \<^bold>=\<^sup>L y) \<^bold>\<rightarrow>   (Affect(x,y,Q) \<^bold>\<rightarrow> \<^bold>\<not> (Affect(y,x,Q)))\<rfloor>"


definition DN_new :: "\<mu> \<Rightarrow> \<sigma>" where
  "DN_new (x) \<equiv>  \<^bold>\<not>( (\<lambda>y.  (\<^bold>\<box> ( \<^bold>\<forall>\<^sup>Ez. \<^bold>\<exists>\<^sup>EQ.  (\<^bold>\<not>(z \<^bold>=\<^sup>L Q)  \<^bold>\<and>  \<^bold>\<not>(y \<^bold>=\<^sup>L Q)  \<^bold>\<and>  \<^bold>\<not>(x \<^bold>=\<^sup>L Q))  \<^bold>\<and> ( \<^bold>\<not>(y \<^bold>=\<^sup>L z) \<^bold>\<rightarrow> (\<^bold>\<diamond> (Affect(y,z,Q)) ) )    )  ))(x)  )"
...
lemma PN1Attempt3: "\<lfloor>PerfectiveV3(\<phi>) \<^bold>\<rightarrow> (Pn1V3 (\<lambda>x. \<^bold>\<box> (\<phi>(x)) ))\<rfloor>"
   nitpick [user_axioms, timeout = 360] oops
 (* proof -
  obtain ii :: i where
  ...
  obtain iib :: "\<mu> \<Rightarrow> i \<Rightarrow> \<mu> \<Rightarrow> i" where
    f8: "\<forall>X0. \<lfloor>\<lambda>X9. \<forall>X10. X9 \<^bold>\<Turnstile> \<^bold>\<diamond> (\<lambda>X15. X15 \<^bold>\<Turnstile> Affect (X0, X10, mma X0 X9 X10)) \<longrightarrow> iib X0 X9 X10 \<^bold>\<Turnstile> (op r X9 \<^bold>\<and> Affect (X0, X10, mma X0 X9 X10))\<rfloor>"
    by moura
  ...
  then obtain mmb :: "(\<mu> \<Rightarrow> i \<Rightarrow> bool) \<Rightarrow> i \<Rightarrow> \<mu>" and iic :: "(\<mu> \<Rightarrow> i \<Rightarrow> bool) \<Rightarrow> i \<Rightarrow> i" and iid :: "(\<mu> \<Rightarrow> i \<Rightarrow> bool) \<Rightarrow> i \<Rightarrow> i" and mmc :: "(\<mu> \<Rightarrow> i \<Rightarrow> bool) \<Rightarrow> i \<Rightarrow> \<mu>" where
    f9: "\<forall>p. \<lfloor>\<lambda>i. iic p i \<^bold>\<Turnstile> op r i \<and> iic p i \<^bold>\<Turnstile> (\<^sup>\<not>p) (mmb p i) \<and> iic p i \<^bold>\<Turnstile> (\<^sup>\<not>DN_new) (mmb p i) \<and> iic p i \<^bold>\<Turnstile> eiw (mmb p i) \<or> i \<^bold>\<Turnstile> Pn1V3 p\<rfloor>"
    by moura
  have "\<forall>m. \<lfloor>\<lambda>i. (i \<^bold>\<Turnstile> \<^bold>\<diamond> (\<lambda>i. \<exists>ma. i \<^bold>\<Turnstile> eiw ma \<and> (\<forall>mb. (\<forall>p. i \<^bold>\<Turnstile> (\<^sup>\<not>p) m \<or> i \<^bold>\<Turnstile> p mb) \<or> (\<forall>p. i \<^bold>\<Turnstile> p mb \<or> i \<^bold>\<Turnstile> (\<^sup>\<not>p) ma) \<or> (\<forall>p. i \<^bold>\<Turnstile> p mb \<or> i \<^bold>\<Turnstile> (\<^sup>\<not>p) m) \<or> i \<^bold>\<Turnstile> (\<^sup>\<not>eiw) mb \<or> \<lfloor>\<lambda>ia. ia \<^bold>\<Turnstile> \<^bold>\<not> (op r i) \<or> ia \<^bold>\<Turnstile> \<^bold>\<not> (Affect (m, ma, mb))\<rfloor>)) \<or> i \<^bold>\<Turnstile> (\<^sup>\<not>DN_new) m) \<and> (i \<^bold>\<Turnstile> DN_new m \<or> \<lfloor>\<lambda>ia. ia \<^bold>\<Turnstile> \<^bold>\<not> (op r i) \<or> (\<forall>ma. ia \<^bold>\<Turnstile> (\<^sup>\<not>eiw) ma \<or> (\<exists>mb. (\<exists>p. ia \<^bold>\<Turnstile> p m \<and> ia \<^bold>\<Turnstile> (\<^sup>\<not>p) mb) \<and> (\<exists>p. ia \<^bold>\<Turnstile> (\<^sup>\<not>p) mb \<and> ia \<^bold>\<Turnstile> p ma) \<and> (\<exists>p. ia \<^bold>\<Turnstile> (\<^sup>\<not>p) mb \<and> ia \<^bold>\<Turnstile> p m) \<and> ia \<^bold>\<Turnstile> eiw mb \<and> ia \<^bold>\<Turnstile> \<^bold>\<diamond> (\<lambda>i. i \<^bold>\<Turnstile> Affect (m, ma, mb))))\<rfloor>)\<rfloor>"
    using refl DN_new_def by blast (* > 1.0 s, timed out *)
  then obtain iie :: "\<mu> \<Rightarrow> i \<Rightarrow> i" and mmd :: "\<mu> \<Rightarrow> i \<Rightarrow> \<mu>" and bbc :: "\<mu> \<Rightarrow> i \<Rightarrow> \<mu> \<Rightarrow> \<mu> \<Rightarrow> i \<Rightarrow> bool" and mme :: "\<mu> \<Rightarrow> i \<Rightarrow> \<mu> \<Rightarrow> \<mu>" and bbd :: "\<mu> \<Rightarrow> i \<Rightarrow> \<mu> \<Rightarrow> \<mu> \<Rightarrow> i \<Rightarrow> bool" and bbe :: "\<mu> \<Rightarrow> i \<Rightarrow> \<mu> \<Rightarrow> \<mu> \<Rightarrow> i \<Rightarrow> bool" and iif :: "\<mu> \<Rightarrow> i \<Rightarrow> \<mu> \<Rightarrow> i" where
    "\<forall>m. \<lfloor>\<lambda>i. (iie m i \<^bold>\<Turnstile> op r i \<and> iie m i \<^bold>\<Turnstile> eiw (mmd m i) \<and> (\<forall>ma. (\<forall>p. iie m i \<^bold>\<Turnstile> (\<^sup>\<not>p) m \<or> iie m i \<^bold>\<Turnstile> p ma) \<or> (\<forall>p. iie m i \<^bold>\<Turnstile> p ma \<or> iie m i \<^bold>\<Turnstile> (\<^sup>\<not>p) (mmd m i)) \<or> (\<forall>p. iie m i \<^bold>\<Turnstile> p ma \<or> iie m i \<^bold>\<Turnstile> (\<^sup>\<not>p) m) \<or> iie m i \<^bold>\<Turnstile> (\<^sup>\<not>eiw) ma \<or> \<lfloor>\<lambda>ia. ia \<^bold>\<Turnstile> \<^bold>\<not> (op r (iie m i)) \<or> ia \<^bold>\<Turnstile> \<^bold>\<not> (Affect (m, mmd m i, ma))\<rfloor>) \<or> i \<^bold>\<Turnstile> (\<^sup>\<not>DN_new) m) \<and> (i \<^bold>\<Turnstile> DN_new m \<or> \<lfloor>\<lambda>ia. ia \<^bold>\<Turnstile> \<^bold>\<not> (op r i) \<or> (\<forall>ma. ia \<^bold>\<Turnstile> (\<^sup>\<not>eiw) ma \<or> (ia \<^bold>\<Turnstile> bbc m ia ma m \<and> ia \<^bold>\<Turnstile> (\<^sup>\<not>bbc m ia ma) (mme m ia ma)) \<and> (ia \<^bold>\<Turnstile> (\<^sup>\<not>bbd m ia ma) (mme m ia ma) \<and> ia \<^bold>\<Turnstile> bbd m ia ma ma) \<and> (ia \<^bold>\<Turnstile> (\<^sup>\<not>bbe m ia ma) (mme m ia ma) \<and> ia \<^bold>\<Turnstile> bbe m ia ma m) \<and> ia \<^bold>\<Turnstile> eiw (mme m ia ma) \<and> iif m ia ma \<^bold>\<Turnstile> (op r ia \<^bold>\<and> Affect (m, ma, mme m ia ma)))\<rfloor>)\<rfloor>"
    using f8 f7 f6 f5 f4 f3 f2 by blast (* > 1.0 s, timed out *)
...
(*theorem PN: "\<lfloor>PerfectiveV3(\<phi>) \<^bold>\<rightarrow> (PerfectiveV3(\<lambda>x. \<^bold>\<box> (\<phi>(x)) ))\<rfloor> "
 ...
 axiomatization where  Ex:  "\<lfloor>PerfectiveV3 A\<rfloor>"

(*IncConsistency Check- Supposedly unprovable with  \<^bold>\<not>(x \<^bold>=\<^sup>L z) \<^bold>\<rightarrow> *)    
lemma False
  by (metis Ex Affectasymm refl sym trans)
    
(*Consistency check *)
lemma True
  nitpick[satisfy, user_axioms, timeout =1500, expect =genuine] oops
